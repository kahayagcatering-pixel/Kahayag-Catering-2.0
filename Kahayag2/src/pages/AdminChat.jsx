...
import { Send, Paperclip, Image, Search, MoreHorizontal, MessageSquare, User, Clock, Check, ArrowLeft, Menu } from 'lucide-react';
...
export default function AdminChat({ user, searchTerm }) {
  ...
  const [activeUser, setActiveUser] = useState(
    location.state?.userId 
      ? { uid: location.state.userId, name: location.state.userName || 'Client' } 
      : null
  );
  ...
  const [showConversationList, setShowConversationList] = useState(!activeUser);
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      const convs = snapshot.docs.map(doc => ({ uid: doc.id, ...doc.data() }));
      setConversations(convs);
      
      // If initializing with location.state but missing name, find it from conversations
      if (location.state?.userId && activeUser && !activeUser.name) {
        const foundConv = convs.find(c => c.uid === location.state.userId);
        if (foundConv?.userName) {
          setActiveUser({ uid: location.state.userId, name: foundConv.userName });
        }
      }
  ...
  const handleSelectUser = (conv) => {
    setActiveUser({ uid: conv.uid, name: conv.userName || 'Client' });
  ...
  return (
    <div className={`${isMobile ? 'h-[calc(100vh-120px)] flex flex-col' : 'h-[calc(100vh-160px)] flex'} bg-white rounded-[40px] overflow-hidden border border-beige-100 shadow-2xl`}>
            ...
            {filteredConversations.length === 0 ? (
              ...
              filteredConversations.map((conv) => (
                <motion.div 
                  ...
                  onClick={() => setActiveUser({ uid: conv.uid, name: conv.userName || 'Client' })}
                ...
                >
                  <div className="flex justify-between items-start mb-2">
                      <div className="flex items-center gap-3 flex-1">
                        <div className="w-12 h-12 rounded-2xl flex items-center justify-center font-bold uppercase bg-beige-100 text-beige-400 flex-shrink-0">
                          {conv.userName?.charAt(0) || 'C'}
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="font-bold text-sm">{conv.userName || 'Client'}</p>
                          <p className="text-[10px] text-beige-500 truncate">
                            {conv.lastMessage}
                          </p>
                        </div>
                      </div>
                      {conv.unreadByAdmin && <span className="w-3 h-3 bg-red-500 rounded-full border-2 border-white flex-shrink-0"></span>}
                  </div>
                  <p className="text-[10px] text-beige-300 px-1">
                    {formatTime(conv.lastMessageAt)}
                  </p>
                </motion.div>
              ))
      ...
      {/* Mobile: Chat View or Desktop: Main Chat Area */}
      {(!isMobile || (isMobile && !showConversationList)) && (
        <div className="flex-1 flex flex-col bg-white">
          {activeUser ? (
            <>
              <header className={`${isMobile ? 'p-4' : 'p-8'} border-b border-beige-50 flex justify-between items-center bg-white z-10 shadow-sm`}>
                <div className="flex items-center gap-3 md:gap-5 flex-1">
                  ...
                  <div className={`${isMobile ? 'w-10 h-10 text-sm' : 'w-14 h-14 text-xl'} bg-beige-100 rounded-3xl flex items-center justify-center text-beige-800 font-bold border border-beige-50`}>
                    {activeUser.name?.charAt(0) || 'C'}
                  </div>
                  <div className="min-w-0">
                    <h3 className={`serif font-bold italic text-beige-900 ${isMobile ? 'text-lg' : 'text-2xl'} truncate`}>{activeUser.name || 'Client'}</h3>
                    <div className="flex items-center gap-2 mt-1">
                       <span className="w-2 h-2 bg-green-500 rounded-full"></span>
                       <span className="text-[10px] font-black text-beige-400 uppercase tracking-widest">Online</span>
                    </div>
                  </div>
                </div>
                {!isMobile && (
            ...
            <div className="h-full flex flex-col items-center justify-center text-center p-8 md:p-20">
              <div className="w-20 h-20 md:w-24 md:h-24 bg-beige-50 rounded-full flex items-center justify-center text-beige-100 mb-6 md:mb-8 border-4 border-dashed border-beige-100">
                 <MessageSquare size={isMobile ? 40 : 48} />
              </div>
              <h3 className="serif text-2xl md:text-3xl mb-3 md:mb-4 italic text-beige-300">Select a Chat</h3>
              <p className="text-beige-400 max-w-sm leading-relaxed font-medium text-sm">Choose a client to start messaging and managing their inquiries.</p>
            </div>
          )}
        </div>
      )}
